...
{
	int angle = atan2((p.y - o.x), (p.x - o.x)) * 180 / PI * ANGLE_PRECISION; //  *ANGLE_PRECISION is hack to have more presition
	if (angle < 0)
	{
...
void print(multiset<Point> &s_p, TGAImage &image, TGAColor color, bool with_angle = false)
{
	...
	cout << endl;
	multiset<Point>::iterator prev_it = s_p.end();
	int i = 0;
	for (multiset<Point>::iterator it = s_p.begin(); it != s_p.end(); ++it)
	{
		...
		if (prev_it != s_p.end())
		{
			line(prev_it->x, prev_it->y, it->x, it->y, image, color);
		}
...
Point find_left(vector<Point> &v_p)
{
	int l = 0;
	for (int i = 1; i < v_p.size(); ++i)
	{
...
vector<Point> create_hull_Jarvis_N2(vector<Point> &v_p)
{
	...
	vector<Point> tmp;
	tmp.reserve(v_p.size());
	tmp.push_back(find_left(v_p));

	Point cur_p = tmp[0];
	int prev_angle = get_angle(cur_p, Point(cur_p.x, cur_p.y - 1));
	...
	while (true)
	{
		...
		for (auto p : v_p)
		{
			int cur_angle = get_angle(cur_p, p);
...
void test_N4_N3()
{
	...
	//for (int i = 0; i < 50; ++i)
	{
		...
		//cout << "hull:" << endl;
		print(v_hull, image, red, true);
		save_image(image, name + "_N3" + ".tga", false);
		image.clear();
		...
		//print(s_p, image, white);
		//cout << "hull:" << endl;
		print(s_hull, image, red, true);
		save_image(image, name + "_N4" + ".tga", false);
		image.clear();
...
void test_N3_N2()
{

	{
		...
		//for (int i = 0; i < 50; ++i)
		{
			...
			//print(s_p, image, white);
			//cout << "hull:" << endl;
			print(v_hull_N3, image, red, true);
			save_image(image, name + "_N3" + ".tga", false);
			image.clear();
			...
			//print(s_p, image, white);
			//cout << "hull:" << endl;
			print(v_hull_N2, image, red, true);
			save_image(image, name + "_N2" + ".tga", false);
			image.clear();
