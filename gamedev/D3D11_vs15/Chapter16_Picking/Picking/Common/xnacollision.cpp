...
// Slightly more efficient than using XMVector3EqualInt.
//-----------------------------------------------------------------------------
static inline BOOL XMVector3AllTrue( DirectX::FXMVECTOR V )
{
    DirectX::XMVECTOR C;
...
//-----------------------------------------------------------------------------
static inline BOOL XMVector3IsUnit( DirectX::FXMVECTOR V )
{
    DirectX::XMVECTOR Difference = XMVector3Length( V ) - XMVectorSplatOne();
...
//-----------------------------------------------------------------------------
static inline BOOL XMQuaternionIsUnit( DirectX::FXMVECTOR Q )
{
    DirectX::XMVECTOR Difference = XMVector4Length( Q ) - XMVectorSplatOne();
...
//-----------------------------------------------------------------------------
static inline BOOL XMPlaneIsUnit( DirectX::FXMVECTOR Plane )
{
    DirectX::XMVECTOR Difference = XMVector3Length( Plane ) - XMVectorSplatOne();
...
//-----------------------------------------------------------------------------
static inline DirectX::XMVECTOR TransformPlane( DirectX::FXMVECTOR Plane, DirectX::FXMVECTOR Rotation, DirectX::FXMVECTOR Translation )
{
    DirectX::XMVECTOR Normal = XMVector3Rotate( Plane, Rotation );
    DirectX::XMVECTOR D = XMVectorSplatW( Plane ) - XMVector3Dot( Normal, Translation );
...
//-----------------------------------------------------------------------------
static inline DirectX::XMVECTOR PointOnLineSegmentNearestPoint( DirectX::FXMVECTOR S1, DirectX::FXMVECTOR S2, DirectX::FXMVECTOR P )
{
    DirectX::XMVECTOR Dir = S2 - S1;
    DirectX::XMVECTOR Projection = ( XMVector3Dot( P, Dir ) - XMVector3Dot( S1, Dir ) );
    DirectX::XMVECTOR LengthSq = XMVector3Dot( Dir, Dir );

    DirectX::XMVECTOR t = Projection * XMVectorReciprocal( LengthSq );
    DirectX::XMVECTOR Point = S1 + t * Dir;

    // t < 0
    DirectX::XMVECTOR SelectS1 = XMVectorLess( Projection, DirectX::XMVectorZero() );
    Point = XMVectorSelect( Point, S1, SelectS1 );

    // t > 1
    DirectX::XMVECTOR SelectS2 = XMVectorGreater( Projection, LengthSq );
...
// (V0, V1, V2).
//-----------------------------------------------------------------------------
static inline DirectX::XMVECTOR PointOnPlaneInsideTriangle( DirectX::FXMVECTOR P, DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, CXMVECTOR V2 )
{
    // Compute the triangle normal.
    DirectX::XMVECTOR N = XMVector3Cross( V2 - V0, V1 - V0 );

    // Compute the cross products of the vector from the base of each edge to 
    // the point with each edge vector.
    DirectX::XMVECTOR C0 = XMVector3Cross( P - V0, V1 - V0 );
    DirectX::XMVECTOR C1 = XMVector3Cross( P - V1, V2 - V1 );
    DirectX::XMVECTOR C2 = XMVector3Cross( P - V2, V0 - V2 );

    // If the cross product points in the same direction as the normal the the
    // point is inside the edge (it is zero if is on the edge).
    DirectX::XMVECTOR Zero = DirectX::XMVectorZero();
    DirectX::XMVECTOR Inside0 = XMVectorGreaterOrEqual( XMVector3Dot( C0, N ), Zero );
    DirectX::XMVECTOR Inside1 = XMVectorGreaterOrEqual( XMVector3Dot( C1, N ), Zero );
    DirectX::XMVECTOR Inside2 = XMVectorGreaterOrEqual( XMVector3Dot( C2, N ), Zero );
...
VOID ComputeBoundingSphereFromPoints( Sphere* pOut, UINT Count, const DirectX::XMFLOAT3* pPoints, UINT Stride )
{
    ...
    DirectX::XMVECTOR DistX = XMVector3Length( DeltaX );

    DirectX::XMVECTOR DeltaY = MaxY - MinY;
    DirectX::XMVECTOR DistY = XMVector3Length( DeltaY );

    DirectX::XMVECTOR DeltaZ = MaxZ - MinZ;
    DirectX::XMVECTOR DistZ = XMVector3Length( DeltaZ );
    ...
    for( UINT i = 0; i < Count; i++ )
    {
        ...
        DirectX::XMVECTOR Dist = XMVector3Length( Delta );
                                             ...
                                             FLOAT m22, FLOAT m23, FLOAT m33, FLOAT e )
{
    ...
    if( XMVector3Equal( vTmp, DirectX::XMVectorZero() ) ) // planar or linear
    {
...
VOID ComputeBoundingOrientedBoxFromPoints( OrientedBox* pOut, UINT Count, const DirectX::XMFLOAT3* pPoints, UINT Stride )
{
    static CONST XMVECTORI32 PermuteXXY =
                 {
                    XM_PERMUTE_0X, XM_PERMUTE_0X, XM_PERMUTE_0Y, XM_PERMUTE_0W //0013
                 };
    static CONST XMVECTORI32 PermuteYZZ =
                 {
                    XM_PERMUTE_0Y, XM_PERMUTE_0Z, XM_PERMUTE_0Z, XM_PERMUTE_0W //1223
                 };
    ...
    for( UINT i = 0; i < Count; i++ )
    {
        ...
        DirectX::XMVECTOR XXY = XMVectorPermute( Point, Point, 0,0,1,3 );
        DirectX::XMVECTOR YZZ = XMVectorPermute( Point, Point, 1,2,2,3 );
    ...
    DirectX::XMVECTOR Det = XMMatrixDeterminant( R );

    if( XMVector4Less( Det, DirectX::XMVectorZero() ) )
    {
    ...
    for( UINT i = 1; i < Count; i++ )
    {
        DirectX::XMVECTOR Point = XMVector3TransformNormal( XMLoadFloat3( ( DirectX::XMFLOAT3* )( ( BYTE* )pPoints + i * Stride ) ),
                                                   InverseR );
                              ...
                              DirectX::FXMVECTOR Translation )
{
    ...
    DirectX::XMVECTOR VectorScale = XMVectorReplicate( Scale );
...
BOOL IntersectPointSphere( DirectX::FXMVECTOR Point, const Sphere* pVolume )
{
    ...
    DirectX::XMVECTOR Radius = XMVectorReplicatePtr( &pVolume->Radius );

    DirectX::XMVECTOR DistanceSquared = XMVector3LengthSq( Point - Center );
...
BOOL IntersectPointOrientedBox( DirectX::FXMVECTOR Point, const OrientedBox* pVolume )
{
    ...
    DirectX::XMVECTOR TPoint = XMVector3InverseRotate( Point - Center, Orientation );
...
BOOL IntersectPointFrustum( DirectX::FXMVECTOR Point, const Frustum* pVolume )
{
    ...
    DirectX::XMVECTOR TPoint = XMVector3InverseRotate( Point - Origin, Orientation );
    ...
    for( INT i = 0; i < 6; i++ )
    {
        DirectX::XMVECTOR Dot = XMVector4Dot( TPoint, Planes[i] );
                           ...
                           FLOAT* pDist )
{
    ...
    // p = Direction ^ e2;
    DirectX::XMVECTOR p = XMVector3Cross( Direction, e2 );

    // det = e1 * p;
    DirectX::XMVECTOR det = XMVector3Dot( e1, p );
    ...
    if( XMVector3GreaterOrEqual( det, Epsilon ) )
    {
        ...
        DirectX::XMVECTOR NoIntersection = XMVectorLess( u, Zero );
        NoIntersection = XMVectorOrInt( NoIntersection, XMVectorGreater( u, det ) );

        // q = s ^ e1;
        DirectX::XMVECTOR q = XMVector3Cross( s, e1 );
    ...
    else if( XMVector3LessOrEqual( det, -Epsilon ) )
    {
        ...
        DirectX::XMVECTOR NoIntersection = XMVectorGreater( u, Zero );
    ...
    DirectX::XMVECTOR inv_det = XMVectorReciprocal( det );
...
BOOL IntersectRaySphere( DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Direction, const Sphere* pVolume, FLOAT* pDist )
{
    ...
    DirectX::XMVECTOR s = XMVector3Dot( l, Direction );

    DirectX::XMVECTOR l2 = XMVector3Dot( l, l );
    ...
    NoIntersection = XMVectorAndInt( XMVectorLess( s, DirectX::XMVectorZero() ), XMVectorGreater( l2, r2 ) );
    ...
    DirectX::XMVECTOR q = XMVectorSqrt( r2 - m2 );
    DirectX::XMVECTOR t1 = s - q;
    DirectX::XMVECTOR t2 = s + q;

    DirectX::XMVECTOR OriginInside = XMVectorLessOrEqual( l2, r2 );
    DirectX::XMVECTOR t = XMVectorSelect( t1, t2, OriginInside );
...
BOOL IntersectRayAxisAlignedBox( DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Direction, const AxisAlignedBox* pVolume, FLOAT* pDist )
{
    ...
    DirectX::XMVECTOR IsParallel = XMVectorLessOrEqual( XMVectorAbs( AxisDotDirection ), Epsilon );

    // Test against all three axii simultaneously.
    DirectX::XMVECTOR InverseAxisDotDirection = XMVectorReciprocal( AxisDotDirection );
    ...
    DirectX::XMVECTOR t_min = XMVectorSelect( XMVectorMin( t1, t2 ), FltMin, IsParallel );
    DirectX::XMVECTOR t_max = XMVectorSelect( XMVectorMax( t1, t2 ), FltMax, IsParallel );
    ...
    DirectX::XMVECTOR NoIntersection = XMVectorGreater( XMVectorSplatX( t_min ), XMVectorSplatX( t_max ) );

    // if ( t_max < 0.0f ) return FALSE;
    NoIntersection = XMVectorOrInt( NoIntersection, XMVectorLess( XMVectorSplatX( t_max ), DirectX::XMVectorZero() ) );

    // if (IsParallel && (-Extents > AxisDotOrigin || Extents < AxisDotOrigin)) return FALSE;
    DirectX::XMVECTOR ParallelOverlap = XMVectorInBounds( AxisDotOrigin, Extents );
...
BOOL IntersectRayOrientedBox( DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Direction, const OrientedBox* pVolume, FLOAT* pDist )
{
    ...
    // Compute the dot product againt each axis of the box.
    DirectX::XMVECTOR AxisDotOrigin = XMVector3Dot( R.r[0], TOrigin );
    ...
    DirectX::XMVECTOR AxisDotDirection = XMVector3Dot( R.r[0], Direction );
...
BOOL IntersectTriangleTriangle( DirectX::FXMVECTOR A0, DirectX::FXMVECTOR A1, DirectX::FXMVECTOR A2, CXMVECTOR B0, CXMVECTOR B1, CXMVECTOR B2 )
{
    ...
    DirectX::XMVECTOR N1 = XMVector3Cross( A1 - A0, A2 - A0 );

    // Assert that the triangle is not degenerate.
    XMASSERT( !XMVector3Equal( N1, Zero ) );

    // Test points of B against the plane of A.
    DirectX::XMVECTOR BDist = XMVector3Dot( N1, B0 - A0 );
    ...
    UINT BDistIsZeroCR;
    DirectX::XMVECTOR BDistIsZero = XMVectorGreaterR( &BDistIsZeroCR, Epsilon, XMVectorAbs( BDist ) );
    BDist = XMVectorSelect( BDist, Zero, BDistIsZero );

    UINT BDistIsLessCR;
    DirectX::XMVECTOR BDistIsLess = XMVectorGreaterR( &BDistIsLessCR, Zero, BDist );

    UINT BDistIsGreaterCR;
    DirectX::XMVECTOR BDistIsGreater = XMVectorGreaterR( &BDistIsGreaterCR, BDist, Zero );
    ...
    DirectX::XMVECTOR N2 = XMVector3Cross( B1 - B0, B2 - B0 );

    // Assert that the triangle is not degenerate.
    XMASSERT( !XMVector3Equal( N2, Zero ) );

    // Test points of A against the plane of B.
    DirectX::XMVECTOR ADist = XMVector3Dot( N2, A0 - B0 );
    ADist = XMVectorSelect( ADist, XMVector3Dot( N2, A1 - B0 ), SelectY );
    ADist = XMVectorSelect( ADist, XMVector3Dot( N2, A2 - B0 ), SelectZ );

    // Ensure robustness with co-planar triangles by zeroing small distances.
    UINT ADistIsZeroCR;
    DirectX::XMVECTOR ADistIsZero = XMVectorGreaterR( &ADistIsZeroCR, Epsilon, XMVectorAbs( BDist ) );
    ADist = XMVectorSelect( ADist, Zero, ADistIsZero );

    UINT ADistIsLessCR;
    DirectX::XMVECTOR ADistIsLess = XMVectorGreaterR( &ADistIsLessCR, Zero, ADist );

    UINT ADistIsGreaterCR;
    DirectX::XMVECTOR ADistIsGreater = XMVectorGreaterR( &ADistIsGreaterCR, ADist, Zero );
    ...
    //
    DirectX::XMVECTOR ADistIsLessEqual = XMVectorOrInt( ADistIsLess, ADistIsZero );
    DirectX::XMVECTOR ADistIsGreaterEqual = XMVectorOrInt( ADistIsGreater, ADistIsZero );
    ...
    DirectX::XMVECTOR BDistIsLessEqual = XMVectorOrInt( BDistIsLess, BDistIsZero );
    DirectX::XMVECTOR BDistIsGreaterEqual = XMVectorOrInt( BDistIsGreater, BDistIsZero );
    ...
    DirectX::XMVECTOR Dist0 = XMVector3Dot( Delta0, XMVector3Cross( ( BB2 - BB0 ), ( AA2 - AA0 ) ) );
    if( XMVector4Greater( Dist0, Zero ) )
        return FALSE;

    DirectX::XMVECTOR Dist1 = XMVector3Dot( Delta1, XMVector3Cross( ( BB1 - BB0 ), ( AA1 - AA0 ) ) );
...
BOOL IntersectTriangleSphere( DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2, const Sphere* pVolume )
{
    ...
    DirectX::XMVECTOR N = DirectX::XMVector3Normalize( XMVector3Cross( V1 - V0, V2 - V0 ) );

    // Assert that the triangle is not degenerate.
    XMASSERT( !XMVector3Equal( N, DirectX::XMVectorZero() ) );

    // Find the nearest feature on the triangle to the sphere.
    DirectX::XMVECTOR Dist = XMVector3Dot( Center - V0, N );

    // If the center of the sphere is farther from the plane of the triangle than
    // the radius of the sphere, then there cannot be an intersection.
    DirectX::XMVECTOR NoIntersection = XMVectorLess( Dist, -Radius );
...
BOOL IntersectTriangleAxisAlignedBox( DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2, const AxisAlignedBox* pVolume )
{
    ...
    static CONST XMVECTORI32 Permute0W1Z0Y0X =
                 {
                    XM_PERMUTE_0W, XM_PERMUTE_1Z, XM_PERMUTE_0Y, XM_PERMUTE_0X //0x - 0 0y - 1 0z - 2 0w - 3 1x - 4 1y - 5 1z - 6 1w - 7
                 };
    ...
    // around the triangle).
    DirectX::XMVECTOR TriMin = XMVectorMin( XMVectorMin( V0, V1 ), V2 );
    DirectX::XMVECTOR TriMax = XMVectorMax( XMVectorMax( V0, V1 ), V2 );

    // for each i in (x, y, z) if a_min(i) > b_max(i) or b_min(i) > a_max(i) then disjoint
    DirectX::XMVECTOR Disjoint = XMVectorOrInt( XMVectorGreater( TriMin, BoxMax ), XMVectorGreater( BoxMin, TriMax ) );
    if( XMVector3AnyTrue( Disjoint ) )
        return FALSE;

    // Test the plane of the triangle.
    DirectX::XMVECTOR Normal = XMVector3Cross( V1 - V0, V2 - V0 );
    DirectX::XMVECTOR Dist = XMVector3Dot( Normal, V0 );
    ...
    DirectX::XMVECTOR NormalSelect = XMVectorGreater( Normal, Zero );
    DirectX::XMVECTOR V_Min = XMVectorSelect( BoxMax, BoxMin, NormalSelect );
    DirectX::XMVECTOR V_Max = XMVectorSelect( BoxMin, BoxMax, NormalSelect );

    // if n dot v_min + d > 0 || n dot v_max + d < 0 then disjoint
    DirectX::XMVECTOR MinDist = XMVector3Dot( V_Min, Normal );
    DirectX::XMVECTOR MaxDist = XMVector3Dot( V_Max, Normal );

    DirectX::XMVECTOR NoIntersection = XMVectorGreater( MinDist, Dist );
    ...
    Axis = XMVectorPermute( e0, -e0, 3,6,1,0 );
    ...
    Axis = XMVectorPermute( e1, -e1, 3,6,1,0 );
    p0 = XMVector3Dot( TV0, Axis );
    ...
    Axis = XMVectorPermute( e2, -e2, 3,6,1,0 );
    p0 = XMVector3Dot( TV0, Axis );
    p1 = XMVector3Dot( TV1, Axis );
    ...
    Axis = XMVectorPermute( e0, -e0, 2,3,4,1 );
    p0 = XMVector3Dot( TV0, Axis );
    // p1 = XMVector3Dot( V1, Axis ); // p1 = p0;
    p2 = XMVector3Dot( TV2, Axis );
    Min = XMVectorMin( p0, p2 );
    ...
    Axis = XMVectorPermute( e1, -e1, 2,3,4,1 );
    p0 = XMVector3Dot( TV0, Axis );
    p1 = XMVector3Dot( TV1, Axis );
    // p2 = XMVector3Dot( V2, Axis ); // p2 = p1;
    Min = XMVectorMin( p0, p1 );
    ...
    Axis = XMVectorPermute( e2, -e2, 2,3,4,1 );
    p0 = XMVector3Dot( TV0, Axis );
    p1 = XMVector3Dot( TV1, Axis );
    // p2 = XMVector3Dot( V2, Axis ); // p2 = p0;
    Min = XMVectorMin( p0, p1 );
    ...
    Axis = XMVectorPermute( e0, -e0, 5,0,3,2 );
    p0 = XMVector3Dot( TV0, Axis );
    // p1 = XMVector3Dot( V1, Axis ); // p1 = p0;
    p2 = XMVector3Dot( TV2, Axis );
    Min = XMVectorMin( p0, p2 );
    ...
    Axis = XMVectorPermute( e1, -e1, 5, 0, 3, 2);
    p0 = XMVector3Dot( TV0, Axis );
    p1 = XMVector3Dot( TV1, Axis );
    // p2 = XMVector3Dot( V2, Axis ); // p2 = p1;
    Min = XMVectorMin( p0, p1 );
    ...
    Axis = XMVectorPermute( e2, -e2, 5, 0, 3, 2);
    p0 = XMVector3Dot( TV0, Axis );
    p1 = XMVector3Dot( TV1, Axis );
    // p2 = XMVector3Dot( V2, Axis ); // p2 = p0;
    Min = XMVectorMin( p0, p1 );
...
BOOL IntersectTriangleOrientedBox( DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2, const OrientedBox* pVolume )
{
    ...
    DirectX::XMVECTOR TV0 = XMVector3InverseRotate( V0 - Center, Orientation );
    DirectX::XMVECTOR TV1 = XMVector3InverseRotate( V1 - Center, Orientation );
    DirectX::XMVECTOR TV2 = XMVector3InverseRotate( V2 - Center, Orientation );
...
BOOL IntersectSphereSphere( const Sphere* pVolumeA, const Sphere* pVolumeB )
{
    ...
    DirectX::XMVECTOR RadiusA = XMVectorReplicatePtr( &pVolumeA->Radius );

    // Load B.
    DirectX::XMVECTOR CenterB = XMLoadFloat3( &pVolumeB->Center );
    DirectX::XMVECTOR RadiusB = XMVectorReplicatePtr( &pVolumeB->Radius );

    // Distance squared between centers.    
    DirectX::XMVECTOR Delta = CenterB - CenterA;
    DirectX::XMVECTOR DistanceSquared = XMVector3LengthSq( Delta );
...
BOOL IntersectSphereAxisAlignedBox( const Sphere* pVolumeA, const AxisAlignedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR SphereRadius = XMVectorReplicatePtr( &pVolumeA->Radius );
    ...
    DirectX::XMVECTOR LessThanMin = XMVectorLess( SphereCenter, BoxMin );
    DirectX::XMVECTOR GreaterThanMax = XMVectorGreater( SphereCenter, BoxMax );
    ...
    DirectX::XMVECTOR d2 = XMVector3Dot( d, d );
...
BOOL IntersectSphereOrientedBox( const Sphere* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    // Compute d for each dimension.
    DirectX::XMVECTOR LessThanMin = XMVectorLess( SphereCenter, -BoxExtents );
    DirectX::XMVECTOR GreaterThanMax = XMVectorGreater( SphereCenter, BoxExtents );
...
BOOL IntersectAxisAlignedBoxAxisAlignedBox( const AxisAlignedBox* pVolumeA, const AxisAlignedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR Disjoint = XMVectorOrInt( XMVectorGreater( MinA, MaxB ), XMVectorGreater( MinB, MaxA ) );
...
BOOL IntersectOrientedBoxOrientedBox( const OrientedBox* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR t = XMVector3InverseRotate( B_cent - A_cent, A_quat );
    ...
    DirectX::XMVECTOR AR0X = XMVectorAbs( R0X );
    DirectX::XMVECTOR AR1X = XMVectorAbs( R1X );
    DirectX::XMVECTOR AR2X = XMVectorAbs( R2X );

    // Absolute value of columns.
    DirectX::XMVECTOR ARX0 = XMVectorAbs( RX0 );
    DirectX::XMVECTOR ARX1 = XMVectorAbs( RX1 );
    DirectX::XMVECTOR ARX2 = XMVectorAbs( RX2 );
    ...
    DirectX::XMVECTOR NoIntersection = XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX0, -RX0, 3,6,1,0 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX0, ARX0, 3,2,1,0 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR0X, AR0X, 3,2,1,0 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX1, -RX1, 3,6,1,0 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX1, ARX1, 3,2,1,0 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR0X, AR0X, 2,3,0,1) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX2, -RX2, 3,6,1,0 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX2, ARX2, 3,2,1,0 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR0X, AR0X, 1,0,3,2 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX0, -RX0, 2,3,4,1) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX0, ARX0, 2,3,0,1 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR1X, AR1X, 3,2,1,0 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX1, -RX1, 2,3,4,1 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX1, ARX1, 2,3,0,1 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR1X, AR1X, 2,3,0,1 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX2, -RX2, 2,3,4,1 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX2, ARX2, 2,3,0,1 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR1X, AR1X, 1,0,3,2 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX0, -RX0,  5,0,3,2 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX0, ARX0, 1,0,3,2 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR2X, AR2X, 3,2,1,0 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX1, -RX1, 5,0,3,2 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX1, ARX1, 1,0,3,2 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR2X, AR2X, 2,3,0,1 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
    ...
    d = XMVector3Dot( t, XMVectorPermute( RX2, -RX2, 5,0,3,2 ) );
    d_A = XMVector3Dot( h_A, XMVectorPermute( ARX2, ARX2, 1,0,3,2 ) );
    d_B = XMVector3Dot( h_B, XMVectorPermute( AR2X, AR2X, 1,0,3,2 ) );
    NoIntersection = XMVectorOrInt( NoIntersection, 
                                    XMVectorGreater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) );
...
INT IntersectTriangleFrustum( DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2, const Frustum* pVolume )
{
    ...
    DirectX::XMVECTOR TV0 = XMVector3InverseRotate( V0 - Origin, Orientation );
    DirectX::XMVECTOR TV1 = XMVector3InverseRotate( V1 - Origin, Orientation );
    DirectX::XMVECTOR TV2 = XMVector3InverseRotate( V2 - Origin, Orientation );
    ...
    for( INT i = 0; i < 6; i++ )
    {
        DirectX::XMVECTOR Dist0 = XMVector3Dot( TV0, Planes[i] );
        DirectX::XMVECTOR Dist1 = XMVector3Dot( TV1, Planes[i] );
        DirectX::XMVECTOR Dist2 = XMVector3Dot( TV2, Planes[i] );

        DirectX::XMVECTOR MinDist = XMVectorMin( Dist0, Dist1 );
        MinDist = XMVectorMin( MinDist, Dist2 );
        DirectX::XMVECTOR MaxDist = XMVectorMax( Dist0, Dist1 );
        MaxDist = XMVectorMax( MaxDist, Dist2 );

        DirectX::XMVECTOR PlaneDist = XMVectorSplatW( Planes[i] );
    ...
    DirectX::XMVECTOR Near = XMVectorReplicatePtr( &pVolume->Near );
    DirectX::XMVECTOR Far = XMVectorReplicatePtr( &pVolume->Far );
    ...
    for( INT i = 1; i < 8; i++ )
    {
        DirectX::XMVECTOR Temp = XMVector3Dot( Corners[i], Normal );
    ...
    for( INT i = 0; i < 3; i++ )
    {
        for( INT j = 0; j < 6; j++ )
        {
            ...
            DirectX::XMVECTOR Axis = XMVector3Cross( TriangleEdgeAxis[i], FrustumEdgeAxis[j] );

            // Find the min/max of the projection of the triangle onto the axis.
            DirectX::XMVECTOR MinA, MaxA;

            DirectX::XMVECTOR Dist0 = XMVector3Dot( V0, Axis );
            DirectX::XMVECTOR Dist1 = XMVector3Dot( V1, Axis );
            DirectX::XMVECTOR Dist2 = XMVector3Dot( V2, Axis );
            ...
            for( INT k = 1; k < 8; k++ )
            {
                DirectX::XMVECTOR Temp = XMVector3Dot( Axis, Corners[k] );
...
INT IntersectSphereFrustum( const Sphere* pVolumeA, const Frustum* pVolumeB )
{
    ...
    DirectX::XMVECTOR Radius = XMVectorReplicatePtr( &pVolumeA->Radius );
    ...
    DirectX::XMVECTOR Intersects = XMVectorFalseInt();
    ...
    for( INT i = 0; i < 6; i++ )
    {
        ...
        DirectX::XMVECTOR InsideFace = XMVectorTrueInt();
    ...
    DirectX::XMVECTOR Near = XMVectorReplicatePtr( &pVolumeB->Near );
    DirectX::XMVECTOR Far = XMVectorReplicatePtr( &pVolumeB->Far );
    ...
    for( INT i = 0; i < 12; i++ )
    {
        ...
        DirectX::XMVECTOR DistSq = XMVector3Dot( Delta, Delta );
...
INT IntersectOrientedBoxFrustum( const OrientedBox* pVolumeA, const Frustum* pVolumeB )
{
    ...
    for( INT i = 0; i < 6; i++ )
    {
        ...
        DirectX::XMVECTOR Dist = XMVector4Dot( Center, Planes[i] );
        ...
        // axes of the box.
        DirectX::XMVECTOR Radius = XMVector3Dot( Planes[i], R.r[0] );
    ...
    // Test against box axes (3)
    {
        ...
        for( INT i = 1; i < 8; i++ )
        {
            DirectX::XMVECTOR Temp = XMVector3Dot( Corners[i], R.r[0] );
        ...
        DirectX::XMVECTOR BoxDist = XMVector3Dot( Center, R.r[0] );
        ...
        DirectX::XMVECTOR Result = XMVectorOrInt( XMVectorGreater( FrustumMin, BoxDist + Extents ),
    ...
    for( INT i = 0; i < 3; i++ )
    {
        ...
        {
            // Compute the axis we are going to test.
            DirectX::XMVECTOR Axis = XMVector3Cross( R.r[i], FrustumEdgeAxis[j] );
            ...
            DirectX::XMVECTOR Dist = XMVector3Dot( Center, Axis );

            // Project the axes of the box onto the axis to find the "radius" of the box.
            DirectX::XMVECTOR Radius = XMVector3Dot( Axis, R.r[0] );
...
INT IntersectFrustumFrustum( const Frustum* pVolumeA, const Frustum* pVolumeB )
{
    ...
    DirectX::XMVECTOR NearA = XMVectorReplicatePtr( &pVolumeA->Near );
    DirectX::XMVECTOR FarA = XMVectorReplicatePtr( &pVolumeA->Far );
    ...
    for( INT i = 0; i < 6; i++ )
    {
        ...
        for( INT j = 1; j < 8; j++ )
        {
            DirectX::XMVECTOR Temp = XMVector3Dot( AxisB[i], CornersA[j] );
    ...
    DirectX::XMVECTOR NearB = XMVectorReplicatePtr( &pVolumeB->Near );
    DirectX::XMVECTOR FarB = XMVectorReplicatePtr( &pVolumeB->Far );
    ...
    for( INT i = 0; i < 6; i++ )
    {
        ...
        for( INT j = 1; j < 8; j++ )
        {
            DirectX::XMVECTOR Temp = XMVector3Dot( AxisA[i], CornersB[j] );
            Min = XMVectorMin( Min, Temp );
        }
    ...
    for( INT i = 0; i < 6; i++ )
    {
        ...
        {
            // Compute the axis we are going to test.
            DirectX::XMVECTOR Axis = XMVector3Cross( FrustumEdgeAxisA[i], FrustumEdgeAxisB[j] );
            ...
            for( INT k = 1; k < 8; k++ )
            {
                DirectX::XMVECTOR TempA = XMVector3Dot( Axis, CornersA[k] );
                MinA = XMVectorMin( MinA, TempA );
                MaxA = XMVectorMax( MaxA, TempA );

                DirectX::XMVECTOR TempB = XMVector3Dot( Axis, CornersB[k] );
...
//-----------------------------------------------------------------------------
static inline void FastIntersectTrianglePlane( DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2, CXMVECTOR Plane,
                                        XMVECTOR& Outside, XMVECTOR& Inside )
{
    // Plane0
    DirectX::XMVECTOR Dist0 = XMVector4Dot( V0, Plane );
    DirectX::XMVECTOR Dist1 = XMVector4Dot( V1, Plane );
    DirectX::XMVECTOR Dist2 = XMVector4Dot( V2, Plane );
...
{
    DirectX::XMVECTOR One = XMVectorSplatOne();

    // Set w of the points to one so we can dot4 with a plane.
    DirectX::XMVECTOR TV0 = XMVectorInsert(V0, One, 0, 0, 0, 0, 1);
    DirectX::XMVECTOR TV1 = XMVectorInsert(V1, One, 0, 0, 0, 0, 1);
    DirectX::XMVECTOR TV2 = XMVectorInsert(V2, One, 0, 0, 0, 0, 1);
...
//-----------------------------------------------------------------------------
static inline void FastIntersectSpherePlane( DirectX::FXMVECTOR Center, DirectX::FXMVECTOR Radius, DirectX::FXMVECTOR Plane,
                                      XMVECTOR& Outside, XMVECTOR& Inside )
{
    DirectX::XMVECTOR Dist = XMVector4Dot( Center, Plane );
...
//-----------------------------------------------------------------------------
static inline void FastIntersectAxisAlignedBoxPlane( DirectX::FXMVECTOR Center, DirectX::FXMVECTOR Extents, DirectX::FXMVECTOR Plane,
                                              XMVECTOR& Outside, XMVECTOR& Inside )
{
    // Compute the distance to the center of the box.
    DirectX::XMVECTOR Dist = XMVector4Dot( Center, Plane );
    ...
    DirectX::XMVECTOR Radius = XMVector3Dot( Extents, XMVectorAbs( Plane ) );
...
//-----------------------------------------------------------------------------
static inline void FastIntersectOrientedBoxPlane( DirectX::FXMVECTOR Center, DirectX::FXMVECTOR Extents, DirectX::FXMVECTOR Axis0, CXMVECTOR Axis1,
                                          CXMVECTOR Axis2, CXMVECTOR Plane, XMVECTOR& Outside, XMVECTOR& Inside )
{
    ...
    // where h(i) are extents of the box, n is the plane normal, and b(i) are the 
    // axes of the box.
    DirectX::XMVECTOR Radius = XMVector3Dot( Plane, Axis0 );
...
//-----------------------------------------------------------------------------
static inline void FastIntersectFrustumPlane( DirectX::FXMVECTOR Point0, DirectX::FXMVECTOR Point1, DirectX::FXMVECTOR Point2, CXMVECTOR Point3,
                                      ...
                                      CXMVECTOR Plane, XMVECTOR& Outside, XMVECTOR& Inside )
{
    ...
    DirectX::XMVECTOR PlaneDist = -XMVectorSplatW( Plane );
