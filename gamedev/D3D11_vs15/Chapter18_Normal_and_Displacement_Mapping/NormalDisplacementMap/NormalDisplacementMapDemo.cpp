...
bool NormalDisplacementMapApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/floor.dds", &texRes, &mStoneTexSRV ));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/bricks.dds", &texRes, &mBrickTexSRV ));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/floor_nmap.dds", &texRes, &mStoneNormalTexSRV));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/bricks_nmap.dds", &texRes, &mBrickNormalTexSRV ));
	ReleaseCOM(texRes);
