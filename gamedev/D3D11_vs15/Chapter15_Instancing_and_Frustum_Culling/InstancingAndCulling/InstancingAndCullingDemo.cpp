...
void InstancingAndCullingApp::UpdateScene(float dt)
{
	...
	{
		DirectX::XMVECTOR detView = XMMatrixDeterminant(mCam.View());
		DirectX::XMMATRIX invView = XMMatrixInverse(&detView, mCam.View());
...
void InstancingAndCullingApp::BuildSkullGeometryBuffers()
{
	...
	XMStoreFloat3(&mSkullBox.Center, DirectX::XMVectorScale(DirectX::XMVectorAdd(vMin,vMax), 0.5f));
	XMStoreFloat3(&mSkullBox.Extents, DirectX::XMVectorScale(DirectX::XMVectorSubtract(vMax,vMin), 0.5f));
