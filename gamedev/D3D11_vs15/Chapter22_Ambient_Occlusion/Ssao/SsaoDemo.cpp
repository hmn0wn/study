...
bool SsaoApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/floor.dds", 0, &mStoneTexSRV ));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/bricks.dds", 0, &mBrickTexSRV ));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/floor_nmap.dds", 0, &mStoneNormalTexSRV));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, 
		L"Textures/bricks_nmap.dds", 0, &mBrickNormalTexSRV ));
...
void SsaoApp::BuildShadowTransform()
{
	...
	DirectX::XMVECTOR lightPos = DirectX::XMVectorScale(lightDir,-2.f*mSceneBounds.Radius);
