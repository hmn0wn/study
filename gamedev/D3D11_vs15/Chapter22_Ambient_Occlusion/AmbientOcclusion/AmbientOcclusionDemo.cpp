	...
	const std::vector<UINT>& indices)
{
	...
	for(UINT i = 0; i < tcount; ++i)
	{
		...
		DirectX::XMVECTOR edge0 = DirectX::XMVectorSubtract(v1, v0);
		DirectX::XMVECTOR edge1 = DirectX::XMVectorSubtract(v2, v0);

		DirectX::XMVECTOR normal = DirectX::XMVector3Normalize(DirectX::XMVector3Cross(edge0, edge1));

		DirectX::XMVECTOR centroid = DirectX::XMVectorScale(DirectX::XMVectorAdd(v0 , DirectX::XMVectorAdd(v1, v2)),1.f/3.0f);

		// Offset to avoid self intersection.
		centroid = DirectX::XMVectorAdd(centroid,DirectX::XMVectorScale(normal, 0.001f));
