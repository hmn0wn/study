...
void compute_density(sim_state_t* s, sim_param_t* params)
{
    ...
    const float* __restrict x = s->prev_pos;
    float h = params->h;
    float h2 = h*h;
...
void compute_accel(sim_state_t* state, sim_param_t* params)
{
    ...
    const float gx = params->gx;
    const float gy = params->gy;

    const float mass = state->mass;
    const float h2 = h*h;
    // Unpack system state
    const float* __restrict rho = state->rho;
    const float* __restrict x = state->prev_pos;
...
sim_state_t* place_particles(sim_param_t* param, domain_fun_t indicatef)
{
    ...
    sim_state_t* s = alloc_state(count, param->hcount);
    int p = 0;
    for (float x = 0; x < 1; x += hh) {
        for (float y = 0; y < 1; y += hh) {
            if (indicatef(x, y)) {
...
{
    sim_param_t *params = new sim_param_t;
...
void write_param(std::ofstream &fout, sim_param_t* params)
{
    ...
    fout << "gravitational strength : " << params->gx << std::endl;
...
void init_particle_system(sim_param_t *params, sim_state_t *state)
{
    ...
    for (int i = 0; i < params->hcount; ++i)
    {
        update_particle_system(params, state);
    }
}
void update_particle_system(sim_param_t *params, sim_state_t *state)
{
    ...
    if (state->cur_pos == (state->pos + state->n * (params->hcount - 1)* 2))
    {
        state->cur_pos = state->pos;
    }
...
{
    return state->cur_pos[state->n * layer + 2 * pos + 0];
    
}

float get_y(sim_state_t *state, int pos, int layer)
{
    return state->cur_pos[state->n * layer + 2 * pos + 1];
}